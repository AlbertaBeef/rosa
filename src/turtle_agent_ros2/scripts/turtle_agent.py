...
class TurtleAgent(ROSA):
    ...
    def __init__(self, node: Node, streaming: bool = False, verbose: bool = True):
        ...
        #    quantization        Q4_0
        #self.__llm = ChatOllama( model="llama2", streaming=streaming, temperature=0, num_ctx=4096 )
        #╭─────────────────────────────── Final Response ───────────────────────────────╮
        #│ An error occurred: registry.ollama.ai/library/llama2:latest does not support │
        ...
        #    embedding length    4096      
        #    quantization        Q4_K_M 
        #self.__llm = ChatOllama( model="llama3.1", streaming=streaming, temperature=0, num_ctx=131072 )
        ...
        #    quantization        Q4_K_M
        #self.__llm = ChatOllama( model="qwen2.5-coder", streaming=streaming, temperature=0, num_ctx=32192 )        
        ...
        #    embedding length    3584        
        #    quantization        Q4_K_M  
        #self.__llm = ChatOllama( model="qwen2.5vl", streaming=streaming, temperature=0, num_ctx=128000 )        
        # ╭─────────────────────────────── Final Response ───────────────────────────────╮
        # │ An error occurred: registry.ollama.ai/library/qwen2.5vl:latest does not      │
        # │ support tools (status code: 400)                                             │
        # ╰──────────────────────────────────────────────────────────────────────────────╯

        #$ ollama show hhao/qwen2.5-coder-tools
        #  Model
        #    architecture        qwen2     
        #    parameters          7.6B      
        #    context length      32768     
        #    embedding length    3584      
        #    quantization        Q4_K_M  
        #self.__llm = ChatOllama( model="hhao/qwen2.5-coder-tools", streaming=streaming, temperature=0, num_ctx=32192 )        
        # ... not using the tools, just describing how to do it ... 
        ...
        #    embedding length    4096      
        #    quantization        Q4_K_M         
        #self.__llm = ChatOllama( model="mistral", streaming=streaming, temperature=0, num_ctx=32192 )        
        # ... not using tools, really slow ...
...
def main():
    ...
    rclpy.init()
    node = rclpy.create_node("rosa")

    turtle_tools.node = node
    turtle_tools.setup_default_pub()

    streaming = node.declare_parameter('streaming', False).value
    turtle_agent = TurtleAgent(node=node, verbose=False, streaming=streaming)
