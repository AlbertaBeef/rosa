...
# You'll need to pass the node instance to these functions or use a singleton.
# For simplicity, let's assume a global node instance for now.
node: Node = None  # This will be set by your main script.
...
def add_cmd_vel_pub(name: str):
    ...
    cmd_vel_pubs[name] = node.create_publisher(Twist, topic, 10)

def remove_cmd_vel_pub(name: str):
    global cmd_vel_pubs
    #cmd_vel_pubs.pop(name, None)
    pub = cmd_vel_pubs.pop(name, None)
    if pub:
        pub.destroy()
...
# Add the default turtle1 publisher on startup (will be called after node init)
def setup_default_pub():
...
def kill_turtle(names: List[str]):
    ...
    for name in names:
        ...
        client = node.create_client(Kill, f"/{name}/kill")
        if not client.wait_for_service(timeout_sec=5.0):
            response += f"Failed to kill {name}: /{name}/kill service not available.\n"
            continue
        req = Kill.Request()
        ...
        if future.result() is not None:
            ...
            response += f"Failed to kill {name}: {future.exception()}\n"
...
def clear_turtlesim():
    ...
    if future.result() is not None:
        ...
        return f"Failed to clear the turtlesim background: {future.exception()}"
...
def get_turtle_pose(names: List[str]) -> dict:
    ...
    for name in names:
        ...
        msg = None
        try:
            msg = rclpy.task.Future()
            def pose_callback(pose_msg):
                msg.set_result(pose_msg)
            sub = node.create_subscription(Pose, f"/{name}/pose", pose_callback, 1)
            rclpy.spin_until_future_complete(node, msg, timeout_sec=5.0)
            poses[name] = msg.result()
...
def teleport_absolute(
    ...
    if future.result() is not None:
        ...
        return f"Failed to teleport the turtle: {future.exception()}"
...
def reset_turtlesim():
    ...
    if future.result() is not None:
        ...
        return f"Failed to reset the turtlesim environment: {future.exception()}"
...
def set_pen(name: str, r: int, g: int, b: int, width: int, off: int):
    ...
    if future.result() is not None:
        ...
        return f"Failed to set the pen color for the turtle: {future.exception()}"
